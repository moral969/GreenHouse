...
int watrDay = 2;  // переменная частоты полива в днях Э каждые watrDay дня Э
...
int timeoutM = 5, timeoutH = 0, timeoutS = 0; // переменная в минутах. таймаут пополнение бочки. на случай выхода из строя геркона.

int wHrs = 18, wMin = 0, wSec = 0; // тут записано время полива МОЖНО КОРРЕКТИРОВАТЬ 
int WDHrs = 0, WDMin = 5, WDSec = 0;  //продолжительность полива МОЖНО КОРРЕКТИРОВАТЬ
...
uint32_t FinishB = 60000, FinishW = 60000; // переменные время в МС по истечению которого отключается насаос МОЖНО КОРРЕКТИРОВАТЬ. // 60 000 = 1 минуте // 300 000 = 5 минутам // 600 000 = 10мин
